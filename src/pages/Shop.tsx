...
const SERAGAM_ASN = [
  { id: 'hitamputih', title: 'Seragam Seleksi CAT', description: 'Pakaian hitam putih formal pejuang CPNS saat tes.', cost: 400, filter: 'grayscale brightness-110' },
  { id: 'korpri', title: 'Batik Korpri Biru', description: 'Pakaian batik korpri resmi kebanggaan ASN abdi negara.', cost: 600, filter: 'hue-rotate-[220deg] saturate-125' },
  { id: 'pdh_kemendagri', title: 'Baju Dinas PDH Cokelat', description: 'Pakaian dinas cokelat harian kementerian berwibawa.', cost: 800, filter: 'brightness-95 contrast-105 saturate-110' },
];
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'buy' ? (
          ...
          >
            ...
            {/* Seragam Dinas ASN (Avatars) Section */}
            <section className="pt-4">
              <h2 className="text-lg md:text-xl font-bold mb-4 md:mb-6 flex items-center gap-2 text-skd-text border-b border-skd-border pb-2">
                <Sparkles className="text-skd-premium" size={24} /> Kustomisasi Seragam ASN (Baju Dinas)
              </h2>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {SERAGAM_ASN.map((seragam) => {
                  const isUnlocked = profile?.unlocked_avatars?.includes(seragam.id);

                  return (
                    <motion.div
                      key={seragam.id}
                      whileHover={{ scale: 1.03 }}
                      className="bg-skd-card border border-skd-border p-5 rounded-3xl flex flex-col items-center gap-4 text-center transition-all shadow-sm hover:shadow-md relative overflow-hidden group"
                    >
                      {/* Visual Avatar Preview */}
                      <div className="relative w-24 h-24 rounded-full overflow-hidden border-4 border-skd-border shadow-inner">
                        <img 
                          src={avatarPdh} 
                          alt={seragam.title} 
                          className={`w-full h-full object-cover ${seragam.filter}`}
                        />
                        {isUnlocked && (
                          <div className="absolute inset-0 bg-skd-success/80 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <div className="flex-1">
                        <h3 className="font-bold text-base text-skd-text mb-1">{seragam.title}</h3>
                        <p className="text-xs text-skd-muted leading-relaxed max-w-xs">{seragam.description}</p>
                      </div>

                      {isUnlocked ? (
                        <div className="w-full py-2.5 bg-skd-success/10 text-skd-success text-sm font-bold rounded-xl border border-skd-success/20 flex items-center justify-center gap-1.5">
                          <Check size={16} /> Sudah Dimiliki
                        </div>
                      ) : (
                        <button
                          onClick={() => handlePurchase(seragam.id, seragam.title, seragam.cost, true)}
                          className="w-full py-2.5 bg-gradient-to-r from-skd-premium to-skd-accent hover:from-purple-500 hover:to-amber-500 text-white text-sm font-black rounded-xl shadow-lg transition-all flex items-center justify-center gap-2"
                        >
                          <Coins size={16} className="text-yellow-300 fill-yellow-300" />
                          <span>{seragam.cost} Koin</span>
                        </button>
                      )}
                    </motion.div>
                  );
